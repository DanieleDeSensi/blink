...
int main(int argc, char** argv){
    ...
    /* one full gathered block per batched (granularity) call so the concurrently
     * outstanding MPI_Iallgather ops never share a receive buffer (-grty > 1).  */
        ...
        for(k=0;k<max_iters+warm_up_iters;k++){
            ...
            do{
                ...
                for(i=0;i<measure_granularity;i++){
                    MPI_Iallgather(send_buf,msg_size,MPI_BYTE,&recv_buf[(size_t)i*w_size*msg_size],msg_size,MPI_BYTE,MPI_COMM_WORLD,&requests[i]);
                }
