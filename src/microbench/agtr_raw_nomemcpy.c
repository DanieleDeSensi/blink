                   ...
                   void* rbuf, size_t rcount, MPI_Datatype rdtype, MPI_Comm comm)
{
  ...
  double ret_time = MPI_Wtime() - start_time;
        ...
        for(k=0;k<max_iters+warm_up_iters;k++){
            ...
            do{
                ...
                for(i=0;i<measure_granularity;i++){
                    remove_time = allgather_ring(send_buf, msg_size_ints, MPI_INT, recv_buf, msg_size_ints, MPI_INT, MPI_COMM_WORLD);
                }
                durations[curr_iters%max_samples]=MPI_Wtime()-measure_start_time-remove_time; /*write result to buffer (lru space)*/
                curr_iters++;
