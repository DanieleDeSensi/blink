class app:
    ...
    # data parameters:
    path_to_executable = './src/ember/mpi/pingpong/pingpong'
    num_metrics = 6  # (int) specifies how many datapoints are collected
    # (list (of length num_metrics) of strings)
    data_labels = ['MsgSize', 'Time', 'Msgs',
                   'Bytes', 'Msg-Throughput', 'Throughput']
    # (list (of length num_metrics) of strings)
    data_units = ['B', 's', 'KMsgs', 'MB', 'KMsg/s', 'MB/s']
    conv_mask = [False, True, False, False, False, False]
