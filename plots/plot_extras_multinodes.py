...
extra_fullname["SL0_hcoll0"] = "SL0 hcoll0"
extra_fullname["SL0_hcoll1"] = "SL0 hcoll1"
extra_fullname["SL1_hcoll0"] = "SL1 hcoll0"
extra_fullname["SL1_hcoll1"] = "SL1 hcoll1"
    ...
    for metric_hr in args.metrics.split(","):      
        ...
        for nodes in args.numnodes.split(","):
            ...
            if args.ppn == "DEFAULT_MULTINODE":
                ppn = get_default_multinode_ppn(args.system, args.victim_name)
            else:
                ppn = int(args.ppn)
            ...
            for extra in args.extras.split(","):
                ...
                for actual_metric in actual_metrics:
                    filename = get_data_filename(args.data_folder, args.system, nodes, args.allocation_mode, allocation_split, ppn, get_actual_extra_name(extra, args.system, vn, nodes), vn, args.victim_input, args.aggressor_name, args.aggressor_input)
                    data = pd.DataFrame()
                    if filename and os.path.exists(filename):                    
                        data[actual_metric] = get_bench_data(vn, args.victim_input, actual_metric, filename, ppn, nodes, args.system)
                    else:
        ...
        if "box" in plot_types:
            ax = sns.boxplot(data=global_df, x="Extra", y=metric_hr, hue="Nodes", hue_order=labels)            
            ax.legend_.set_title(None)
            ax.set_xlabel("")
            ax.set_ylabel(add_unit_to_metric(metric_hr))
        ...
        if "bar" in plot_types:
            ax = sns.barplot(data=global_df, x="Extra", y=metric_hr, hue="Nodes", hue_order=labels)
            ax.legend_.set_title(None)
            ax.set_xlabel("")
            ax.set_ylabel(add_unit_to_metric(metric_hr))
