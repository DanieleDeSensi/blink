class app:
    ...
    #data parameters:
    path_to_executable='./apps/internals/inc_bsnbr/inc_bsnbr'#(string) path to executable
    num_metrics=5 #(int) specifies how many datapoints are collected
    data_labels=['Avg-Duration','Min-Duration','Max-Duration','Median-Duration','Receiver-Duration']#(list (of length num_metrics) of strings)
    data_units=['s','s','s','s','s'] #(list (of length num_metrics) of strings)
