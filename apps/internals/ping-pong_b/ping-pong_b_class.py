class app:
    ...
    #data parameters:
    path_to_executable='./apps/internals/ping-pong_b/ping-pong_b'#(string) path to executable
    num_metrics=1 #(int) specifies how many datapoints are collected
    data_labels=['Duration']#(list (of length num_metrics) of strings)
    data_units=['s'] #(list (of length num_metrics) of strings)
    conv_mask=[True]
