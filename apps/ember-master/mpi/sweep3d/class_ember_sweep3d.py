class app:
    ...
    #data parameters:
    path_to_executable='./apps/ember-master/mpi/sweep3d/sweep3d'#(string) path to executable
    num_metrics=3 #(int) specifies how many datapoints are collected
    data_labels=['Time','MaxData_xchanged/rank','Throughput/rank']#(list (of length num_metrics) of strings)
    data_units=['s','KB/rank','MB/s/Rank'] #(list (of length num_metrics) of strings)
    conv_mask=[True,True,True]
