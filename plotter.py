...
# Time/iteration on the x-axis
def plot_trend_line(df, title, metric, outname, trend_limit, max_y):
    # Setup the plot
...
def main():
    ...
    parser.add_argument('-a', '--aggressors_info', help='Comma-separated list of aggressors name:input_name (e.g. a2a:128,inc:1024). Names must match the filename of the Python wrapper.', default="")
    ...
    parser.add_argument('-e', '--extra', help='Any extra info about the execution (comma-separated list).', default="")
    ...
    victim_inputs = []
    if args.victim_info.count(":") > 1 and args.aggressors_info.count(",") == 0:
    ...
    with open(args.data_folder + "/description.csv", mode='r') as infile:
        ...
        for line in reader:
            ...
            if row["system"] == args.system and row["numnodes"] == args.num_nodes and \
               ...
               int(row["ppn"]) == int(args.ppn) and row["extra"] == args.extra:                               
                ...
                if "aggressor_wrapper" in info:
                    ...
                    aggressor_fn = ""
                    aggressor_in = ""
                    aggressors_found = (args.aggressors_info == "")
    ...
    if plot_trend_inputs:
        ...
        if(args.aggressors_info != "" and len(data_filename) != len(aggressors_input)):
            raise Exception("Error: could not find all the data files (or too much data has been found)")
    ...
    for metric in args.metrics.split(","):        
        ...
        if plot_trend_inputs:
            ...
            plot_trend_line(df, victim_name_h, metric, outname, args.trend_limit, args.max_y)  
        else:
            df = load_data_distribution(args.aggressors_info, data_filename, metric)

            # Violins        
            plot_violin(df, victim_fullname, metric, outname, args.max_y)

            # Boxes        
            plot_box(df, victim_fullname, metric, outname, args.max_y)

            # Lines
            plot_line(df, victim_fullname, metric, outname, args.max_y)
