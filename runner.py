...
def check_CI(container_list,alpha,beta,converge_all,run):
    ...
    for container in container_list:
        if (not container.converged) and (converge_all or container.conv_goal): #not converged yet and has to converge
...
def c_allocation(apps,nodes_frame):
    for app in apps:
        nodes_list=list(filter(lambda ele: ele!='',nodes_frame[:][app.id_num].tolist()))
...
def log_meta_data(out_format,path,data_container_list,num_runs):
    num_samples_index=list(map(lambda x: 'Run '+str(x),list(range(1,num_runs+1))))
    dataframe=pandas.DataFrame()
    for i,container in enumerate(data_container_list):
        dataframe=pandas.concat([dataframe,pandas.DataFrame(container.md_to_list())],axis=1,ignore_index=True)
    dataframe.index=['App_id','Label','Unit','Convergence_goal','Converged','Converged_run']+num_samples_index
    if out_format=='csv':
        file_name=path+'.csv'
        dataframe.to_csv(file_name,index=True)
    elif out_format=='hdf':
        file_name=path+'.h5'
        dataframe.to_hdf(file_name,key='df',data_columns=data_header_cols,index=False)
            
def log_data(out_format,path,data_container_list):
    dataframe=pandas.DataFrame()
    columns=[]
    for i,container in enumerate(data_container_list):
        dataframe=pandas.concat([dataframe,pandas.DataFrame(container.data)],axis=1,ignore_index=True)
        columns+=[container.get_title()]
...
def main():
    ...
    parser.add_argument('-mn','--minruns',help='Minimum number of runs.',default=10,type=int)
    parser.add_argument('-mx','--maxruns',help='Maximum number of runs.',default=1000,type=int)
    parser.add_argument('-t','--timeout',help='Maximum duration of testing.',default=100.0,type=float)
    ...
    parser.add_argument('-ca','--convergeall',help='Test until all metrics converged.',action='store_true',default=False)
    ...
    parser.add_argument('-d','--datapath',help='Seed for randomness',default='./data')
        ...
        for line in schedule_file:
            ...
            if line_list[4]=='':
                ...
                if end_time<start_time:
                    raise Exception('Application '+str(num_apps)+' must be started before it can be killed.')
            ...
            #import app class
            app_class_name=(import_path_app.split(os.sep)[-1])[:-3] #assumes name is last element of path without .py suffix
    ...
    #import wlm class
    wlm_class_name=(import_path_wlm.split(os.sep)[-1])[:-3] #assumes name is last element of path without .py suffix
    ...
    if allocation_mode=='c': #custom allocation, doesn't need splits
        if num_cols!=num_apps:
            raise Exception('Must specify exactly one column of nodes for every application in '+node_file+'.')
    ...
    for app in apps:
        print_runtime(str(app.id_num)+' on '+str(app.num_nodes)+' nodes:',ro_mode,ro_file)
    ...
    for app_id, action_type, action_time in schedule:
        if action_type=='s':
            print_runtime(str(action_time)+'s: start '+str(app_id),ro_mode,ro_file)
        elif action_type=='k':
            print_runtime(str(action_time)+'s: kill '+str(app_id),ro_mode,ro_file)
        elif action_type=='t':
            print_runtime(str(action_time)+'s: terminate '+str(app_id),ro_mode,ro_file)
    ...
    #end of preps
    print_runtime('\nPreparing took '+str(round(time.time()-pre_start_time,5))+'s.',ro_mode,ro_file)
    ...
    while True:
        ...
        if exec_time>=time_out: #termination condition timeout inbetween schedules
            ...
            break;
        if(time_out-(time.time()-start_time)<(schedule[-1])[2]): #not enough time to finish another run of the schedule
        ...
        for app_id, action_type, action_time in schedule:
            ...
            if action_type=='s':
                ...
                end_job(apps[app_id])
                print_runtime('  '+str(round(time.time()-run_start_time,5))+'s: killed '+str(app_id),ro_mode,ro_file)
            elif action_type=='t':
                end_job(apps[app_id])
                print_runtime('  '+str(round(time.time()-run_start_time,5))+'s: terminated '+str(app_id),ro_mode,ro_file)
            else:
        ...
        for app_ind in apps_awaited:
            ...
            if timeout_occured:
                print_runtime('  '+str(round(time.time()-run_start_time,5))+'s: timed out '+str(app_ind),ro_mode,ro_file)
            else:
                print_runtime('  '+str(round(time.time()-run_start_time,5))+'s: awaited '+str(app_ind),ro_mode,ro_file)
        ...
        if timeout_occured:
            ...
            print_runtime('  No data collection due to timeout.',ro_mode,ro_file)
            print_runtime('Completed after timeout, terminated after '+str(runs)+' runs taking '
        ...
        for app in apps:
            if (app.num_metrics > 0) and app.collect_flag:
                ...
                for data_list in data_list_of_list:
                    ...
                    j+=1
        print_runtime('  Data collection took '+str(round(time.time()-data_collection_start_time,5))+'s.',ro_mode,ro_file)
        
        #check convergence
        if runs>=min_runs:
            conv_check_start_time=time.time()
            converged=check_CI(data_container_list,alpha,beta,converge_all,runs)
            print_runtime('  Convergence check took '+str(round(time.time()-conv_check_start_time,5))+'s.',ro_mode,ro_file)
    
    #create logs
    log_start_time=time.time()
    log_data(out_format,data_directory+'/d_'+runner_id,data_container_list)
    log_meta_data(out_format,data_directory+'/md_'+runner_id,data_container_list,runs)
    print_runtime('Writing data & meta-data took '+str(round(time.time()-log_start_time,5))+'s.',ro_mode,ro_file)
    
    print_runtime('Overall took '+str(round(time.time()-pre_start_time,5))+'s.',ro_mode,ro_file)
